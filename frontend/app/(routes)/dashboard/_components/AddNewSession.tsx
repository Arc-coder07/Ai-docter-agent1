...
function AddNewSession({ isOpen, onOpenChange, preSelectedDoctor }: AddNewSessionProps) {
  ...
  const OnClickNext = async () => {
    ...
    try {
      const response = await apiClient("/suggest-doctors", {
        method: "POST",
        data: { notes: note }
      });
  ...
  const handleStartConsultation = async () => {
    ...
    const result = await apiClient("/chat", {
      ...
      data: {
        ...
        // selectedDoctor is processed by frontend UI mainly, backend stores session.
        // If backend needs doctor info, we should pass it. 
        // Current chat endpoint takes 'messages' and optionally 'sessionId'.
        // We are starting a new session here.
        // Let's pass the note as the first message to init the AI.
      }
    })

    // Backend returns { reply: ..., sessionId: ... }
