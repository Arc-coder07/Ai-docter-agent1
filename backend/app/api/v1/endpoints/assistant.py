...
async def download_diagnosis_report(
    ...
    """
    Generate and download a PDF report for any assistant session.
    Includes conversation log, uploaded images, and AI analysis.
    """
    ...
    if not messages:
        raise HTTPException(status_code=400, detail="No messages found in this session")

    # Build conversation messages list for the PDF
    conversation_msgs = []
    for m in messages:
        msg_dict = {
            "role": m.role,
        ...
        if m.image_url:
            # image_url is like "/uploads/backend/filename.jpg"
            relative_path = m.image_url.lstrip("/")
            abs_path = os.path.join(BASE_DIR, relative_path)
            if os.path.exists(abs_path):
                msg_dict["image_path"] = abs_path
        conversation_msgs.append(msg_dict)

    # For image analysis sessions, also grab the primary medical image + heatmap
    image_path = ""
    heatmap_path = ""
    image_agents = ["BRAIN_TUMOR_AGENT", "CHEST_XRAY_AGENT", "SKIN_LESION_AGENT"]
    is_image_session = any(agent in (session.agent_used or "") for agent in image_agents)
    ...
    if is_image_session:
        ...
        # Check for Grad-CAM heatmap (generated by X-ray agent)
